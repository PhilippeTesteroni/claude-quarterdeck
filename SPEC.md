...
- **R-25.3 Lamp behavior.** Blur/Esc never hide the lamp (it is the point of pinning). New ask/perm while in lamp mode: lamp does NOT auto-expand (the ask window handles urgency); it just reflects status. Tooltip on hover: same counts line as the popup footer.
- **R-25.4 Onboarding refresh (consolidates v1.1/v1.2).** The R-10.2 onboarding card is updated to cover the grown surface, still ONE screen: hooks install (unchanged), agent questions (unchanged), NEW "Take over permission prompts" consent line (R-16.4, default-on checkbox with one-sentence explanation), launch-at-login question (unchanged), and a closing tip line "Pin the window (📌) and click ◱ to shrink it to a traffic light." Settings pane gains the corresponding toggles it does not yet show (takeoverPermissions, showTokenStats). Any FUTURE feature that adds a consent or a persistent mode MUST state its onboarding/settings impact in its spec section (standing rule).
- **R-25.5 Tests.** Mode persistence round-trip, lamp aggregate correctness vs engine worst-of (mock), badge count, expand-in-place geometry, unpin-from-lamp path, onboarding card renders all consent items (Playwright), reduced-motion lamp.
